...
class PinPasswordValidator final : public PasswordValidatorDecorator
{
public:
    explicit PinPasswordValidator(std::unique_ptr<PasswordValidator> validator)
        : PasswordValidatorDecorator{ std::move(validator) } {}
    ...
    bool validate(std::string password) const override {
        ...
        // not correct
        return password.find_first_not_of("123456") == std::string::npos;
    }
};

class PinLengthValidator final : public PasswordValidatorDecorator
{
private:
    size_t m_maxLength;

public:
    explicit PinLengthValidator(std::unique_ptr<PasswordValidator> validator, size_t maxLength)
        : PasswordValidatorDecorator{ std::move(validator) }, m_maxLength{ maxLength } {}
    ...
    bool validate(std::string password) const override
    {
        ...
        return password.length() <= m_maxLength;
    }
};

void validating_passwords_with_pin()
{
    std::unique_ptr<PasswordValidator> pinValidator{
        std::make_unique<PinLengthValidator>(
            std::make_unique<PinPasswordValidator>(
                std::make_unique<LengthValidator>(4)), 4)
    };

    bool valid = pinValidator->validate("1234");

    if (valid) {
        std::cout << "Gutes Passwort" << std::endl;
    }
    else {
